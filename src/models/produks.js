...
const produks = {
   getAll: (nama) => {
        return new Promise((resolve,reject)=>{
            db.query(`select produk.id , produk.name , produk.price , categories.category_name ,produk.image from produk INNER JOIN categories ON produk.category_id = categories.id where name LIKE '%${nama}%' ORDER BY price DESC`,(err,result)=> {
